...
always2_inv_part (\<lambda> s. getPstate s p_ProblemController' = p_ProblemController's_ctrl')
...
always2_inv_part (\<lambda> s. getPstate s p_ProblemController' = STOP)
...
P1_1_part (v_IDLE_TIMEOUT'TIMEOUT - 1) (\<lambda> s1. getVarBool s1 v_pSensor' = False) (\<lambda> s2. getVarBool s2 v_engine' = True \<and> getVarBool s2 v_pSensor' = False)
...
P1_1_inv_part (v_IDLE_TIMEOUT'TIMEOUT - 1) (\<lambda> s. if getPstate s p_Carousel' = p_Carousel's_turnedOn' then ltime s p_Carousel' - 1 else 0)
...
P4_2_inv_part (v_IDLE_TIMEOUT'TIMEOUT - 1) (\<lambda> s. getPstate s p_ProblemController' = STOP) 
  (\<lambda> s. if getPstate s p_ProblemController' = STOP then (v_IDLE_TIMEOUT'TIMEOUT - 1) else ltime s p_Carousel' - 1)
   (\<lambda> s1. getVarBool s1 v_engine' = True) (\<lambda> s2. getVarBool s2 v_pSensor' = v_DETECTED') (\<lambda> s3. getVarBool s3 v_engine' = True) 
  (\<lambda> s4. getVarBool s4 v_stuck' = v_DETECTED' \<or> getVarBool s4 v_weight' = v_HIGH') s"
