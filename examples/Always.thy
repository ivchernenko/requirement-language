...
lemma always_inv_saving_gen: "always_inv A' s \<Longrightarrow> consecutive s s' \<Longrightarrow>  always_imp s (A' s) (A' s') \<and> A' s' s' \<Longrightarrow>
 always_inv A' s'"
  apply(unfold always_inv_def always_imp_def)
  apply simp
  by (metis One_nat_def substate_noteq_imp_substate_of_pred)

lemma always_einv_imp_req_gen: "always_inv A' s  \<Longrightarrow> toEnvP s \<Longrightarrow> always_imp s (A' s) (A s) \<Longrightarrow> always A s"
...
lemma always_inv_saving: "always_inv_part A s \<Longrightarrow> consecutive s s' \<Longrightarrow>   A s' \<Longrightarrow>
 always_inv_part A s'"
  ...
  apply(simp add: always_inv_saving_gen always_imp_refl)
  done
...
lemma always_einv_imp_req: "always_inv_part A s  \<Longrightarrow> toEnvP s \<Longrightarrow> True \<Longrightarrow> always_part A s"
  ...
  apply(simp add: always_einv_imp_req_gen always_imp_refl)
  done
