...
(getPstate s p_Lighting' \<in>{ p_Lighting's_night', STOP} \<longrightarrow> getVarBool s v_dayLight' = False \<and> getVarBool s v_dimLight' = False \<and>
   getVarInt s v_timeOfDay' = v_NIGHT') \<and>
(getPstate s p_Lighting'  \<in>{ p_Lighting's_night', STOP} \<longrightarrow> getPstate s p_Motion' = STOP) \<and>
