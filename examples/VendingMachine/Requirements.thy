...
P3_4_inv_part (\<lambda> s. getPstate s p_Controller' \<noteq> p_Controller's_waiting') (\<lambda> s. getPstate s p_Controller' \<noteq> p_Controller's_choice')
   (\<lambda> s. getPstate s p_Sale1' \<in> {STOP, ERROR})
   (\<lambda> s1. getVarInt s1 v_mode'= v_IDLE') (\<lambda> s2. getVarInt s2 v_mode' = v_CHOICE') (\<lambda> s3. getVarBool s3 v_product1' = False)
 (\<lambda> s4. getVarBool s4 v_button1' = True) s"
...
always2_inv_part (\<lambda> s. getPstate s p_Sale1' = p_Sale1's_delivery')
   (\<lambda> s1. getVarBool s1 v_product1' = False) (\<lambda> s2. getVarInt s2 v_credit' < v_PRICE1')
 (\<lambda> s2. getVarBool s2 v_product1' = False) s"
...
P3_4_inv_part (\<lambda> s. getPstate s p_Controller' \<noteq> p_Controller's_choice') (\<lambda> s. getPstate s p_Sale1' \<noteq> p_Sale1's_addMoney') 
  (\<lambda> s. getPstate s p_Sale1' = p_Sale1's_addMoney')
   (\<lambda> s1. getVarInt s1 v_mode' = v_CHOICE')
 (\<lambda> s2. getVarBool s2 v_cancel' = False \<and> getVarBool s2 v_button1' = v_PRESSED' \<and> getVarInt s2 v_credit' < v_PRICE1')
(\<lambda> s3. getVarBool s3 v_lockChanger' = False \<and> getVarInt s3 v_mode' = v_ADD_MONEY')
 (\<lambda> s4. getVarBool s4 v_cancel' = True \<or> getVarInt s4 v_credit' \<ge> v_PRICE1') s"
...
P1_2_inv_part 2 (\<lambda> s . getPstate s p_Sale1' \<noteq> p_Sale1's_delivery')
  (\<lambda> s. if getPstate s p_Controller' = p_Controller's_sale' \<and> getPstate s p_Sale1' = STOP \<and> getPstate s p_Sale2' = STOP then 1
else if getPstate s p_Controller' = p_Controller's_payOut' \<and> getVarBool s v_dispense' = False then 2 else 0)
   (\<lambda> s1. getVarBool s1 v_product1' = True) (\<lambda> s2. getVarBool s2 v_given1' = True \<and> getVarInt s2 v_credit' > 0)
(\<lambda> s3. getVarBool s3 v_dispense' = False \<and> getVarBool s3 v_paidOut' = False)
 (\<lambda> s4. getVarBool s4 v_dispense' = True \<and> getVarInt s4 v_change' = getVarInt s4 v_credit' \<or> getVarBool s4 v_paidOut' =True) s"

definition R5 where "R5 s \<equiv>
always_part (\<lambda> s1. getVarInt s1 v_mode' = v_IDLE' \<longrightarrow> getVarBool s1 v_lockChanger' = False) s"

definition Einv5 where "Einv5 s \<equiv> commonExtraInv s \<and>
always_inv_part (\<lambda> s1. getVarInt s1 v_mode' = v_IDLE' \<longrightarrow> getVarBool s1 v_lockChanger' = False) s"

definition R6 where "R6 s \<equiv>
always_part (\<lambda> s1.  getVarInt s1 v_mode' = v_IDLE' \<longrightarrow> getVarInt s1 v_credit' \<le> 0) s"

definition Einv6 where "Einv6 s \<equiv> commonExtraInv s \<and>
always_inv_part (\<lambda> s1.  getVarInt s1 v_mode' = v_IDLE' \<longrightarrow> getVarInt s1 v_credit' \<le> 0) s"
...
always2_inv_part
   (\<lambda> s. getPstate s p_Controller' \<noteq> p_Controller's_waiting' \<and> getPstate s p_Sale1' \<noteq> p_Sale1's_addMoney' \<and>
    getPstate s p_Sale2' \<noteq> p_Sale2's_addMoney')
   (\<lambda> s1. getVarBool s1 v_lockChanger' = False) (\<lambda> s2. True)
 (\<lambda>  s2. let s1 = pred s2 in toEnvP s1 \<and> getVarInt s2 v_credit' = getVarInt s1 v_credit' + getVarInt s2 v_deposited') s"
...
always2_inv_part (\<lambda> s. getPstate s p_Sale1' \<noteq> p_Sale1's_delivery')
   (\<lambda> s1. getVarBool s1 v_product1' = True) (\<lambda> s2. getVarBool s2 v_given1' = True)
 (\<lambda> s2. let s1 = pred s2 in toEnvP s1 \<and> getVarInt s2 v_credit' = getVarInt s1 v_credit' - v_PRICE1') s"
...
P1_2_inv_part 1 (\<lambda> s. getPstate s p_Sale1' \<noteq> p_Sale1's_delivery')
  (\<lambda> s. if getPstate s p_Sale1' = ERROR then 1 else 0)
   (\<lambda> s1. getVarBool s1 v_product1' = True) (\<lambda> s2. getVarBool s2 v_product1' = False \<and> getVarBool s2 v_given1' = False)
(\<lambda> s3. getVarInt s3 v_mode' \<noteq> v_CHOICE') (\<lambda> s4. getVarInt s4 v_mode' = v_CHOICE') s"
...
always_inv_part (\<lambda> s1. getVarBool s1 v_product1' = True \<or> getVarBool s1 v_product2' = True \<longrightarrow> getVarBool s1 v_lockChanger' = True) s"
...
always_inv_part (\<lambda> s1. \<not> (getVarBool s1 v_product1' = True \<and> getVarBool s1 v_product2' = True)) s"
