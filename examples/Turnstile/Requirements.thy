theory Requirements
  imports "../Derived_Patterns" Turnstile
begin
...
definition Einv1 where "Einv1 s \<equiv> toEnvP s \<and>
...
definition Einv2 where "Einv2 s \<equiv> toEnvP s \<and>
...
definition Einv3 where "Einv3 s \<equiv> toEnvP s \<and>
always2_inv_part (\<lambda> s. getPstate s Controller' \<in> {Controller'minimalOpened', Controller'isOpened'})
...
definition Einv4 where "Einv4 s \<equiv> toEnvP s \<and>
