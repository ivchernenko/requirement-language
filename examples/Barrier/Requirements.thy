...
P1_1_part v_OPEN_TIME'TIMEOUT  (\<lambda> s1. getVarBool s1 v_green' = True)
   (\<lambda> s3. getVarBool s3 v_down' = False \<and> getVarBool s3 v_peCarUnder' = False \<and> 
     (getVarBool s3 v_carInFront' = False \<or> (let s2 = pred s3 in toEnvP s2 \<longrightarrow> getVarBool s2 v_carInFront' = True)))
   (\<lambda> s5. getVarBool s5 v_down' = True \<or>  getVarBool s5 v_peCarUnder' = True \<or> 
...
P2_2_inv_part  v_OPEN_TIME'TIMEOUT (\<lambda> s. getPstate s p_Opening' = p_Opening's_open')
   (\<lambda> s.
    if getPstate s p_Opening' = p_Opening's_open' then ltime s p_Opening' - 1 
    else if getPstate s p_Opening' = p_Opening's_opening' then 1 else v_OPEN_TIME'TIMEOUT - 1)
...
P2_1_inv_part v_OPEN_TIME'TIMEOUT 
   (\<lambda> s.
    if getPstate s p_Opening' = p_Opening's_open' then ltime s p_Opening' - 1 
    else if getPstate s p_Opening' \<in> {p_Opening's_opening', STOP} then 0 else v_OPEN_TIME'TIMEOUT - 1)
  (\<lambda> s1. getVarBool s1 v_peCarUnder' = True) (\<lambda> s2. getVarBool s2 v_down' = False) s"

definition R8 where "R8 s \<equiv> 
always2_part (\<lambda> s1. getVarBool s1 v_down' = True)
...
always2_inv_part (\<lambda> s. getPstate s p_Opening' \<noteq> p_Opening's_closing')
  (\<lambda> s1. getVarBool s1 v_down' = True)
   (\<lambda> s2. getVarBool s2 v_peCarUnder' = True \<and> getVarBool s2 v_opened' = False \<and> getVarBool s2 v_closed' = False)
   (\<lambda> s2. getVarBool s2 v_up' = True) s"
...
P2_2_inv_part v_OPEN_TIME'TIMEOUT (\<lambda> s. getPstate s p_CarController' = p_CarController's_waitingForCarPassing')
   (\<lambda> s.
    if getPstate s p_Opening' = p_Opening's_open' then ltime s p_Opening' - 1 
    else if getPstate s p_Opening' = p_Opening's_opening' then 0 else v_OPEN_TIME'TIMEOUT - 1)
   (\<lambda> s1. getVarBool s1 v_carInFront' = False)
  (\<lambda> s2. getVarBool s2 v_carInFront' = True \<and> getVarBool s2 v_green' = True) (\<lambda> s3. getVarBool s3 v_down' = False) s"

definition R11 where "R11 s \<equiv> 
always_part (\<lambda> s1. getVarBool s1 v_up' = True \<or> getVarBool s1 v_down' = True \<longrightarrow> getVarBool s1 v_red' = True) s"

definition Einv11 where "Einv11 s \<equiv> commonExtraInv s \<and>
always_inv_part (\<lambda> s1. getVarBool s1 v_up' = True \<or> getVarBool s1 v_down' = True \<longrightarrow> getVarBool s1 v_red' = True) s"
