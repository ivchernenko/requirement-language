...
always2_part (\<lambda> s1. getVarBool s1 v_oSteam' = True \<and> getVarBool s1 v_iLowLevel') (\<lambda> s2. getVarBool s2 v_iHighTemp' = False)
 (\<lambda> s2. getVarBool s2 v_oSteam' = True) s"

definition Einv2 where "Einv2 s \<equiv> common_extra_invariant s \<and>
always2_inv_part (\<lambda> s. getPstate s p_Initialization' = p_Initialization's_begin' \<or>
     getPstate s p_Initialization' = p_Initialization's_waitForSterilization' \<and> getPstate s p_ForceSterilization' = STOP \<or> 
    getPstate s p_Initialization' = p_Initialization's_keepSterilization' \<and> getVarBool s v_oSteam' = False)
   (\<lambda> s1. getVarBool s1 v_oSteam' = True \<and> getVarBool s1 v_iLowLevel') (\<lambda> s2. getVarBool s2 v_iHighTemp' = False) (\<lambda> s2. getVarBool s2 v_oSteam' = True) s"
*)

definition  R3 where "R3 s \<equiv>
R3_pattern_part (60 * 10 - 1) (\<lambda> s1. getVarBool s1 v_oFillTank' = True) (\<lambda> s2. getVarBool s2 v_iHighLevel' = True \<and> getVarBool s2 v_iHighTemp' = False)
   (\<lambda> s3. getVarBool s3 v_iHighTemp' = True) (\<lambda> s4. getVarBool s4 v_oSteam' = True) (\<lambda> s5. False) s"
...
R3_pattern_inv_part (60 * 10 - 1) 
 ...
 (\<lambda> s1. getVarBool s1 v_oFillTank' = True) (\<lambda> s2. getVarBool s2 v_iHighLevel' = True \<and> getVarBool s2 v_iHighTemp' = False)
   (\<lambda> s3. getVarBool s3 v_iHighTemp' = True) (\<lambda> s4. getVarBool s4 v_oSteam' = True) (\<lambda> (s5::state) . False) s"

definition R4 where "R4  s \<equiv>
P1_2_part 1 (\<lambda> s1. getVarBool s1 v_oFillBottle' = True) (\<lambda> s2. getVarBool s2 v_iBottleLevel' = True \<and> getVarBool s2 v_iLowLevel' = False) 
  (\<lambda> s3. getVarBool s3 v_oFillTank' = False \<and> getVarBool s3 v_iHighLevel' = False)
  (\<lambda> s4. getVarBool s4 v_oFillTank' = True \<or> getVarBool s4 v_iHighLevel' = True) s"

definition Einv4 where "Einv4  s \<equiv> common_extra_invariant s \<and>
P1_2_inv_part 1 (\<lambda> s. getPstate s p_MainLoop' \<noteq> p_MainLoop's_waitForFilling') 
  (\<lambda> s. if getPstate s p_Initialization' = p_Initialization's_begin' \<and> getVarBool s v_oFillTank' = False then 1 else 0)
