...
(getPstate s p_AutoDoors' \<notin> {p_AutoDoors's_closing', p_AutoDoors's_closingDelay'} \<longrightarrow> getPstate s p_Closing' = STOP) \<and>
  (getPstate s p_AutoDoors' \<noteq> p_AutoDoors's_opening' \<and> getPstate s p_Closing' \<noteq> p_Closing's_opening' \<longrightarrow> getVarBool s v_open' = False) \<and>
...
getPstate s p_AutoDoors' \<in> 
  ...
  , STOP} \<and>
getPstate s p_Closing' \<in> {p_Closing's_closing', p_Closing's_opening', STOP, ERROR} \<and>
